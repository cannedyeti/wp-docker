jQuery( document ).ready(function( $ ) {
  ...
  var $mobileNav = $('<nav class="mobile-menu" role="navigation"></nav>'),
      $mobileBar = $('<div class="mobile-menu__bar"><button class="mobile-menu__button js-mobile-menu-button mobile-menu__button--menu"><span class="mobile-menu__icon mobile-menu__icon--menu"></span></button></div>'),
  ...
  if ($mainMenu.length > 0) {
    ...
    // get main menu image
    var headerImage = $('.header--main').find('.site-logo__image').attr('src');
    $mobileBar.prepend('<a href="/" class="mobile-header__image" style="background-image: url(' + headerImage + ')"></a>')
    ...
    if (!($('.mobile-menu .mobile-menu__search').length > 0)) {
      if ($('.block--search').length > 0) {
        $('.block--search').clone().addClass('mobile-menu__search').removeClass('block--search').appendTo($mobileNav);
